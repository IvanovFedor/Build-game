...
public class FirstPersonController : MonoBehaviour
{
    ...
    private GameObject PlayerScriptAAA;
    ...
    void Start()
    {
        ...
        {
            PlayerScriptAAA = GameObject.Find("Player");
        }
    ...
    private void Update()
    {
        ...
        if(SceneManager.GetActiveScene().name == "Prolog")
        {
            if (cameraCanMove)
            {
                ...
                if(SceneManager.GetActiveScene().name == "Prolog" && gameObject.name == "Player (2)")
                {
                    
                    float yawClamped = Mathf.Clamp(yaw, 75, 315);
                    
                   
                    transform.localEulerAngles = new Vector3(0, yawClamped, 0);
        ...
        else
        {
            if (cameraCanMove )
            {
                if (Input.GetMouseButton(1))
                {
                    ...
                    if (!invertCamera)
                    {
                        pitch -= mouseSensitivity/8 * Input.GetAxis("Mouse Y");
                    }
                    else
                    {
                        // Inverted Y
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = new Vector3(Input.GetAxis("Horizontal"), -100, Input.GetAxis("Vertical"));
    ...
    public void CoolDown()
    {
        ...
        sprintFOV = PlayerPrefs.GetFloat("Fov") + 5;
        playerCanMove = true;
        isSprinting = true;
    }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.StepsAudio = (AudioSource)EditorGUILayout.ObjectField(new GUIContent("Шаги короче"), fpc.StepsAudio, typeof(AudioSource), true);
